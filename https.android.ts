				...
				try {
					client.hostnameVerifier(
						new javax.net.ssl.HostnameVerifier({
							verify: function(
								hostname: string,
								session: javax.net.ssl.ISSLSession
							): boolean {
...
const strictModeThreadPolicyPermitAll = new android.os.StrictMode.ThreadPolicy.Builder()
	.permitAll()
	.build();
...
export function request({
	...
	return new Promise(function(resolve, reject) {
		try {
			...
			request.url(url);
			Object.keys(mergedHeaders).forEach(key => request.addHeader(key, mergedHeaders[key]));

			const withoutBody =
				['GET', 'HEAD'].includes(method) || (method == 'DELETE' && !isDefined(body));
			...
			return client.newCall(request.build()).enqueue(
