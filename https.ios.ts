...
export function enableSSLPinning(options: Https.HttpsSSLPinningOptions) {
	if (!policies.secure) {
		...
		let allowInvalidCertificates = isDefined(options.allowInvalidCertificates)
			? options.allowInvalidCertificates
			: false;
		policies.secure.allowInvalidCertificates = allowInvalidCertificates;
		let validatesDomainName = isDefined(options.validatesDomainName)
...
function AFFailure(resolve, reject, task: NSURLSessionDataTask, error: NSError) {
	let data: NSData = error.userInfo.valueForKey(
...
export function request(opts: Https.HttpsRequestOptions): Promise<Https.HttpsResponse> {
	...
	}).then(function(AFResponse: { task: NSURLSessionDataTask; content: any; reason?: string }) {
